                    ...
                    <div class="body">
                        <form action="{{ route('admin.doctor.update', $doctor->id) }}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="form-group form-float">
                                <div class="form-line">
                                    <input type="text" value="{{ $doctor->name }}" id="title" class="form-control" name="name">
                            ...
                            <div class="form-group">
                                <label for="image" style="margin-right: 10px;">Present Image</label>
                                <img src="{{ asset('images/doctor/' . $doctor->image) }}" width="100">
                            </div>
                                ...
                                <div class="form-line {{ $errors->has('designation_id') ? 'focused error' : '' }}">
                                    ...
                                    <select name="designation_id" id="category" class="form-control show-tick" data-live-search="true">
                                        ...
                                        @foreach($designations as $designation)
                                            <option value="{{ $designation->id }}" {{ $designation->id == $doctor->designation_id ? 'selected' : '' }}>{{ $designation->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                                ...
                                <div class="form-line {{ $errors->has('degrees') ? 'focused error' : '' }}">
                                    ...
                                    <select name="degrees[]" id="category" class="form-control show-tick" data-live-search="true" multiple>
                                        ...
                                        @foreach($degrees as $degree)
                                            <option
                                            @foreach($doctor->degrees as $doctordegree)
                                                {{ $doctordegree->id == $degree->id ? 'selected' : '' }}
                                            @endforeach
                                ...
                                <div class="form-line {{ $errors->has('specialists') ? 'focused error' : '' }}">
                                    ...
                                    <select name="specialists[]" id="category" class="form-control show-tick" data-live-search="true" multiple>
                                        ...
                                        @foreach($Specialists as $specialist)
                                            <option
                                            @foreach($doctor->specialists as $doctorspecialist)
                                                {{ $doctorspecialist->id == $specialist->id ? 'selected' : '' }}
                                            @endforeach
